  ...
  <div class="relative min-h-screen flex items-center bg-[url('/assets/images/monfond.jpg')] bg-cover bg-center justify-center px-4">
    ...
    <div class="relative z-20 w-full max-w-md backdrop-blur-md bg-transparent p-8 rounded-xl shadow-xl animate-fade-in border border-white/20">
      ...
      <p class="text-sm text-center mt-6 text-white/80">
        ...
        <NuxtLink to="/auth/register" class="text-white hover:underline font-medium">
          S'inscrire
        </NuxtLink>
      </p>
...
const handleLogin = async () => {
  ...
  try {
    ...
    // ✅ Stocker dynamiquement le token JWT dans localStorage
    localStorage.setItem('auth_token', data.access)
    localStorage.setItem('refresh_token', data.refresh)
    localStorage.setItem('user', JSON.stringify(data.user)) // Optionnel : stocker les infos de l'utilisateur

    console.log('✅ Token enregistré dans localStorage')
    router.push('/dashboard/profil') // Redirige vers une page protégée
  } catch (err: any) {
