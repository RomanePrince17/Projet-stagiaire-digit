<template>
  <div :class="['min-h-screen flex relative overflow-x-hidden transition-colors duration-300', themeClass]">
    ...
    <aside
      :class="[
        'w-64 p-6 space-y-4 border-r fixed top-0 bottom-0 z-50 md:z-40 transition-transform duration-300',
        sidebarOpen ? 'translate-x-0' : '-translate-x-full',
        'md:translate-x-0 md:block',
        sidebarClass
      ]"
    >
      <h1 :class="['text-2xl font-bold mb-12 border-b', borderClass]">Digit Stage</h1>
      <nav class="space-y-2">
        <NuxtLink
          ...
          :class="[
            ...
            route.path === item.path
              ? 'bg-gradient-to-r from-[#FF5F36] to-[#6E38E0] text-white font-semibold'
              : ''
          ]"
        >
          <component :is="item.icon" class="w-5 h-5" :class="textClass" />
          <span>{{ item.name }}</span>
        </NuxtLink>
        ...
        <DemandesToggle
          class="mb-80"
          :theme="theme"
          :active="route.path === '/dashboard/demandes'"
        />

        <!-- Déconnexion -->
        <Deconnection :theme="theme" :active="route.path === '/auth/login'" />
      </nav>
    </aside>
    ...
    >
      ...
      <header
        :class="[
          'px-4 md:px-8 py-4 flex justify-between items-center fixed top-0 right-0 z-50 shadow transition-all duration-300',
      ...
      >
        ...
        <div class="flex items-center gap-4 w-full max-w-md">
          <button @click="sidebarOpen = !sidebarOpen" class="md:hidden focus:outline-none" :class="textClass">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </button>
          <!-- <input type="text" placeholder="Search for job, candidates and more..."
            :class="['sm:block px-4 py-2 sm:p-2 rounded w-full focus:outline-none focus:ring-2 focus:ring-indigo-500', inputClass]" /> -->
        </div>
...
<div ref="themeDropdownRef" class="relative">
  ...
  <button
    ...
    class="flex items-center justify-center p-2 rounded hover:bg-gray-400 dark:hover:bg-gray-700 transition focus:outline-none"
  >
  ...
  <transition name="fade">
    <div
      ...
      class="absolute right-0 mt-2 w-36 bg-white dark:bg-gray-800 shadow-lg rounded-lg z-50 overflow-hidden"
    >
      <ul class="py-1">
        <!-- Light -->
        <li>
          <button
            @click="theme = 'light'; themeDropdownOpen = false; localStorage.setItem('theme', 'light')"
            class="w-full px-4 py-2 flex items-center gap-2 text-black dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition"
          >
            <!-- Icône Soleil -->
        ...
        <li>
          <button
            @click="theme = 'dark'; themeDropdownOpen = false; localStorage.setItem('theme', 'dark')"
            class="w-full px-4 py-2 flex items-center gap-2 text-black dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition"
          >
            <!-- Icône Lune -->
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 " fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <!-- Contenu dynamique -->
      <main :class="['p-4 pt-28 md:p-8 space-y-8 h-full overflow-y-auto', mainClass]">
        <slot />
      </main>
    </div>
  </div>
...
import Deconnection from '@/components/deconnection.vue'
...
const sidebarClass = computed(() => theme.value === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200')
const headerClass = computed(() => theme.value === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-100 border-gray-200 text-black')
const mainClass = computed(() => theme.value === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black')
...
const menuBgClass = computed(() => theme.value === 'dark' ? 'bg-gray-700 text-white' : 'bg-gray-100 text-black')
const profileBgClass = computed(() => theme.value === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black')
</script>
...
/* Animation slide-in */
.slide-left-enter-active, .slide-left-leave-active { transition: transform 0.3s ease, opacity 0.3s ease; }
